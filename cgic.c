...
static cgiParseResultType cgiParsePostFormInput() {
  ...
  if (
  ((int) fread(input, 1, cgiContentLength, cgiIn)) != cgiContentLength
  ) {
    return cgiParseIO;
  }  
 ...
 * or cgiParseIO.
 */
static cgiParseResultType afterNextBoundary(
  mpStreamPtr mpp, FILE *outf, char **outP,
  int *bodyLengthP, int first
);

static int readHeaderLine(
  mpStreamPtr mpp, char *attr, int attrSpace,
  char *value, int valueSpace
);

static void decomposeValue(
  char *value, char *mvalue, int mvalueSpace, char **argNames,
...
static cgiUnescapeResultType cgiUnescapeChars(
  char **sp, char *cp, int len
);
  ...
  while (e) {
    ...
    if (cgiFormFileOpen(e->attr, &fp) == cgiFormSuccess) {
      ...
      while (cgiFormFileRead(
        fp, buffer, sizeof(buffer), &got) == cgiFormSuccess
      ) {
...
/* 1377: LcgiFormStringNoNewlines(char *name, char *result, int max) */
...
/* 1736: cgiFormResultType cgiCookieString(
  char *name, char *value, int space
)*/
